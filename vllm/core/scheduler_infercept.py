...
class Scheduler:
    ...
    def __init__(
        ...
        self.predicted_waste: Dict[int, List[float]] = {} # preserve, recompute, swap
    ...
    def passive_discard(self, required_blocks: int) -> int:
        ...
        while discarded < required_blocks:
            ...
            if self.block_manager.block_tables.get(seq.seq_id, None) is None:
                ...
                self.strategy[int(target.request_id)] = 'recompute'
        return discarded
    ...
    def get_strategy_used_by_request(self) -> Dict[int, str]:
        return self.strategy

    def get_predicted_waste(self) -> Dict[int, List[float]]: # preserve, recompute, swap
    ...
    def pause_seq_group(self, seq_group: SequenceGroup, blocks_to_swapout: Dict[int, int], first_n_blocks = -1) -> None:
        ...
        if self.scheduler_config.api_policy == 'H':
            if seq_group.sampling_params.api_exec_time >= self.scheduler_config.heuristic_coef:
                ...
                self.strategy[int(seq_group.request_id)] = 'swap'
            else:
                mode = PreemptionMode.PRESERVE
                self.strategy[int(seq_group.request_id)] = 'preserve'
    ...
    def _schedule_chunk_and_fill(self) -> SchedulerOutputs:
        ...
        if self.scheduler_config.api_policy == 'G':
            ...
            preemption_targets = [seq_group for seq_group, mode in self.paused.values() if (mode is PreemptionMode.PRESERVE and seq_group.predicted_api_remaining_time(now) >= 94e-3) or mode is PreemptionMode.PARTIAL]
