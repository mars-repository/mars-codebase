...
class Sequence:
    ...
    def __init__(
        ...
        self.starvation_quantum = 0
        self.STARVATION_THRESHOLD = 2000
        # self.STARVATION_THRESHOLD = 3000
        # self.STARVATION_THRESHOLD = 100
        self.QUANTUM_VALUE = 1

    def set_starvation_threshold_and_quantum(self, threshold: int, quantum: int):
        self.STARVATION_THRESHOLD = threshold
        self.QUANTUM_VALUE = quantum

    def reset_starvation_paused(self):
        self.starvation_counter = 0
        self.starvation_quantum = 0

    def check_reset_starvation_scheduled(self):
        assert self.status == SequenceStatus.RUNNING

        # when self.startvation_quantum > 0, it is scheduled because we prioritize it. decrease quantum
        if self.starvation_quantum > 0:
            self.starvation_quantum -= 1
        else:
            # scheduled BEFORE we have waited too long. reset counter
            self.starvation_counter = 0
    
    def check_starvation_high_priority(self):
        # we should only do this for waiting requests
        assert self.status == SequenceStatus.WAITING

        if self.starvation_quantum > 0:
            self.starvation_quantum -= 1
            return True
        # no quantum right now. check if we should assign quantum
        if self.starvation_counter >= self.STARVATION_THRESHOLD:
            self.starvation_quantum = self.QUANTUM_VALUE
            self.starvation_counter = 0
        
        return False

    def increment_starvation(self):
        # we should only do this for waiting requests
        assert self.status == SequenceStatus.WAITING
        if self.starvation_quantum == 0:
...
class SequenceGroup:
    ...
    def __init__(
        ...
        self.time_before_processing = -1.0
        self.newest_sort_waste = -1.0
    ...
    def predicted_api_remaining_time(self, now: float):
        return max(1e-9, self.sampling_params.predicted_api_exec_time - (now - self.sampling_params.api_call_time))
