...
template<typename scalar_t>
__global__ void new_reshape_and_cache_kernel(
  const scalar_t* __restrict__ key,     // [num_tokens, num_heads, head_size]
  const scalar_t* __restrict__ value,   // [num_tokens, num_heads, head_size]
  ...
  for (int i = threadIdx.x; i < n; i += blockDim.x) {
    ...
    const int tgt_key_idx = block_idx * block_size * num_heads * head_size
                            + block_offset * num_heads * head_size
                            + head_idx * head_size
                            + head_offset;
    const int tgt_value_idx = block_idx * block_size * num_heads * head_size
                              + block_offset * num_heads * head_size
                              + head_idx * head_size
                              + head_offset;
    key_cache[tgt_key_idx] = __ldg(&key[src_key_idx]);
  ...
  torch::Tensor& slot_mapping)  // [num_tokens]
{
  ...
  VLLM_DISPATCH_FLOATING_TYPES(
    key.scalar_type(),
    "new_reshape_and_cache_kernel",
    [&] {
      vllm::new_reshape_and_cache_kernel<scalar_t><<<grid, block, 0, stream>>>(
        key.data_ptr<scalar_t>(),
        value.data_ptr<scalar_t>(),
        key_cache.data_ptr<scalar_t>(),
        value_cache.data_ptr<scalar_t>(),
